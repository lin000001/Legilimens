		...
		**kwargs,
		...
		if generation_config is None:
			...
			if self.generation_config._from_model_config and self.generation_config._original_object_hash == hash(
				...
				# print("TTTTTTTTT", new_generation_config)
	...
	def greedy_search(
		...
		**model_kwargs,
		...
		while True:
			...
			outputs, next_token_hidden_states, all_layer_hidden_states = self(
				**model_inputs,
				return_dict=True,
				output_attentions=output_attentions,
			...
			# following include all transformer
			all_all_layer_hidden_states.append(all_layer_hidden_states)
			if synced_gpus and this_peer_finished:
				continue  # don't waste resources running the code we don't need

			# extract the feature of first token
			...
			# if next_tokens in eos_token_id_tensor:

			# detect first token
			if len(all_hidden_states) == 2:
				feature = all_all_layer_hidden_states[1][-1]
				
				data = feature.squeeze(0)
				moderator = ThreeLayerClassifier(dim = 4096)
				moderator.load_state_dict(torch.load('./model.pt'), assign = True)
				moderator.cuda()
				output = torch.squeeze(moderator(data.to(torch.float32)),1)
				pred = output.argmax(dim=1)
				print('prediction', pred)

				input_ids[pred == 1]=torch.cat((input_ids[pred == 1][:,:-1], eos_token_id_tensor[0].repeat(pred.sum(),1)),dim=1)
				next_tokens[pred == 1] =  eos_token_id_tensor[0] 
   
   
   
	
				# 	Universal_ids = torch.tensor([306, 29915, 29885,  2289,  7423, 29892, 306, 508, 29915, 29873,  1371, 411, 393, 29889])
				# 	batch_ids = Universal_ids.repeat(20,1)
				# 	output_ids = torch.cat((input_ids[:, :-1], batch_ids.cuda()), dim = 1)
				# 	return output_ids, all_hidden_states, all_all_layer_hidden_states
				# else:
				# 	continue
		...
		if return_dict_in_generate:
			...
			return input_ids, all_hidden_states, all_all_layer_hidden_states
	...
	def sample(
		...
		**model_kwargs,
		...
		while True:
			...
			# if len(all_hidden_states) == 2:
			# 	feature = all_hidden_states[-1]
			# 	moderator = ThreeLayerClassifier(dim = 4096)
			# 	moderator.load_state_dict(torch.load('model.pt'))
			# 	moderator.cuda()
			# 	data = feature.squeeze(0)
